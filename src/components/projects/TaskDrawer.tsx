...
import {
  ...
  Timeline, Input, Button, Checkbox, Divider, Upload, Empty, theme, Tooltip, Badge,
} from 'antd';
import {
  UserOutlined, CalendarOutlined, EyeOutlined, EyeInvisibleOutlined,
  PaperClipOutlined, SendOutlined, UploadOutlined, FireOutlined,
  CheckCircleOutlined, LinkOutlined, ArrowRightOutlined, CloseOutlined,
...
export const TaskDrawer: React.FC<TaskDrawerProps> = ({
  ...
  const [localStatus, setLocalStatus] = useState<string>(task?.status || 'Not Started');
  const [localPriority, setLocalPriority] = useState<string>(task?.priority || 'Medium');
  const [localClientVisible, setLocalClientVisible] = useState<boolean>(task?.clientVisible || false);
  ...
  React.useEffect(() => {
    if (task) {
      ...
      setLocalClientVisible(task.clientVisible);
  ...
  const completedSubtasks = localSubtasks.filter(st => st.status === 'Completed').length;
  const subtaskProgress = localSubtasks.length > 0 ? Math.round((completedSubtasks / localSubtasks.length) * 100) : 0;
  ...
  const detailsTab = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        ...
        <Paragraph style={{ fontSize: 13, marginBottom: 0 }}>{task.description}</Paragraph>
      </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <Select
            ...
            options={[
              ...
              { value: 'Pending Approval', label: <Tag color="warning">Pending Approval</Tag> },
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={{ fontSize: 13 }}>{task.owner}</Text>
          </div>
        </div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <Text style={{ fontSize: 13 }}>{task.startDate}</Text>
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 6 }}>END DATE</Text>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <CalendarOutlined style={{ color: token.colorTextSecondary, fontSize: 12 }} />
            <Text style={{ fontSize: 13 }}>{task.endDate}</Text>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 13 }}>{task.actualHours}h</Text>
        </div>
      </div>
      ...
      {task.isCriticalPath && (
        <div style={{
          ...
          <Text style={{ fontSize: 12, color: '#ff4d4f' }}>This task is on the Critical Path</Text>
        </div>
      )}

      {/* Assigned Resources */}
      {(task.assignedEmployees.length > 0 || task.assignedPartners.length > 0) && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {task.assignedPartners.map(partner => (
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <Text type="secondary" style={{ fontSize: 12 }}>SUBTASKS ({completedSubtasks}/{localSubtasks.length})</Text>
          <Progress percent={subtaskProgress} size="small" style={{ width: 100, margin: 0 }} />
        </div>
            ...
            {localSubtasks.map(st => (
              ...
              >
                ...
                <Text
                  ...
                  style={{
                    fontSize: 12, flex: 1, cursor: onSubtaskClick ? 'pointer' : 'default',
                ...
                <Tag
                  color={st.priority === 'Critical' ? 'red' : st.priority === 'High' ? 'orange' : st.priority === 'Medium' ? 'blue' : 'green'}
                  style={{ fontSize: 10, margin: 0 }}
                >
                  {st.priority}
                </Tag>
                <Text style={{ fontSize: 10, color: token.colorTextSecondary }}>{st.dueDate}</Text>
                <Progress
                  percent={st.status === 'Completed' ? 100 : st.status === 'In Progress' ? 50 : 0}
                  size="small"
                  strokeColor={st.status === 'Completed' ? '#52c41a' : st.status === 'In Progress' ? '#1677ff' : '#d9d9d9'}
                  showInfo={false}
                  style={{ width: 56, margin: 0, flexShrink: 0 }}
                />
              </div>
            ))}
          </div>
        ...
        <>
          ...
          <div>
            ...
            <Select
              ...
              options={allTasks
                .filter(t => t.id !== task?.id && !localDeps.includes(t.id))
          ...
          {/* Blocking */}
          {(() => {
            const blockingTasks = allTasks.filter(t => t.id !== task?.id && t.dependencies.includes(task?.id || ''));
  ...
  const filesTab = (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 40 }}>
      <Empty
        ...
        description={
          <span>
            <Text type="secondary">No files attached to this task</Text>
          </span>
        }
      >
        <Upload>
    ...
    <Drawer
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {task.isCriticalPath && <FireOutlined style={{ color: '#ff4d4f' }} />}
          <span style={{ fontSize: 15 }}>{task.name}</span>
